...
import (
	...
	codegen "github.com/efectn/go-orm-benchmarks/bench/codegen"
...
func ModelsQueryFields(colSet *big.Int) []string {
	...
	fields := []string{}
	if colSet.Bit(codegen.Models_ID) == 1 {
		fields = append(fields, "id")
	}

	if colSet.Bit(codegen.Models_Name) == 1 {
		fields = append(fields, "name")
	}

	if colSet.Bit(codegen.Models_Title) == 1 {
		fields = append(fields, "title")
	}

	if colSet.Bit(codegen.Models_Fax) == 1 {
		fields = append(fields, "fax")
	}

	if colSet.Bit(codegen.Models_Web) == 1 {
		fields = append(fields, "web")
	}

	if colSet.Bit(codegen.Models_Age) == 1 {
		fields = append(fields, "age")
	}

	if colSet.Bit(codegen.Models_Right) == 1 {
		fields = append(fields, "\"right\"")
	}

	if colSet.Bit(codegen.Models_Counter) == 1 {
...
func NewModelsStore(ctx *codegen.Context, conn Execer) *ModelsStore {
	s := &ModelsStore{}
	s.db = conn
	s.withJoin = true
	s.joinType = sdb.LEFT
...
func (s *Models) bind(row []sql.RawBytes, withJoin bool, colSet *big.Int, col *int) {
	if colSet == nil || colSet.Bit(codegen.Models_ID) == 1 {
		s.ID = sdb.ToInt(row[*col])
		*col++
	}
	if colSet == nil || colSet.Bit(codegen.Models_Name) == 1 {
		s.Name = sdb.ToString(row[*col])
		*col++
	}
	if colSet == nil || colSet.Bit(codegen.Models_Title) == 1 {
		s.Title = sdb.ToString(row[*col])
		*col++
	}
	if colSet == nil || colSet.Bit(codegen.Models_Fax) == 1 {
		s.Fax = sdb.ToString(row[*col])
		*col++
	}
	if colSet == nil || colSet.Bit(codegen.Models_Web) == 1 {
		s.Web = sdb.ToString(row[*col])
		*col++
	}
	if colSet == nil || colSet.Bit(codegen.Models_Age) == 1 {
		s.Age = sdb.ToInt(row[*col])
		*col++
	}
	if colSet == nil || colSet.Bit(codegen.Models_Right) == 1 {
		b := sdb.ToUnsafeString(row[*col])
		s.Right = b[0] == 't'

		*col++
	}
	if colSet == nil || colSet.Bit(codegen.Models_Counter) == 1 {
...
func (s *ModelsStore) QueryCustom(stmt string, args ...interface{}) ([]*codegen.Models, error) {
	dto := &Models{}
	data := dto.slice()
	...
	retValues := make([]*codegen.Models, len(data))
	for i := range data {
		retValues[i] = &data[i].Models
	}
	return retValues, nil
}

// One retrieves a row from 'models' as a Models with 1:1 joined data.
func (s *ModelsStore) One(args ...interface{}) (*codegen.Models, error) {
	data := &Models{}
...
func (s *ModelsStore) Query(args ...interface{}) ([]*codegen.Models, error) {
...
func (s *ModelsStore) modelsUpsertStmt() *sdb.UpsertStatement {
	upsert := []string{}
	if s.colSet == nil || s.colSet.Bit(codegen.Models_Name) == 1 {
		upsert = append(upsert, "name = VALUES(name)")
	}
	if s.colSet == nil || s.colSet.Bit(codegen.Models_Title) == 1 {
		upsert = append(upsert, "title = VALUES(title)")
	}
	if s.colSet == nil || s.colSet.Bit(codegen.Models_Fax) == 1 {
		upsert = append(upsert, "fax = VALUES(fax)")
	}
	if s.colSet == nil || s.colSet.Bit(codegen.Models_Web) == 1 {
		upsert = append(upsert, "web = VALUES(web)")
	}
	if s.colSet == nil || s.colSet.Bit(codegen.Models_Age) == 1 {
		upsert = append(upsert, "age = VALUES(age)")
	}
	if s.colSet == nil || s.colSet.Bit(codegen.Models_Right) == 1 {
		upsert = append(upsert, "\"right\" = VALUES(right)")
	}
	if s.colSet == nil || s.colSet.Bit(codegen.Models_Counter) == 1 {
...
func (s *ModelsStore) Upsert(data ...*codegen.Models) (int64, error) {
...
func (s *ModelsStore) Insert(data *codegen.Models) error {
	var err error
	sql := sdb.NewSQLStatement()
...
// nolint[gocyclo]
func (s *ModelsStore) Update(data *codegen.Models) (int64, error) {
	sql := sdb.NewSQLStatement()
	var prepend string
	args := []interface{}{}
...
func (s *ModelsStore) Delete(data *codegen.Models) (int64, error) {
	var err error
...
func (s *ModelsStore) DeleteSlice(data []*codegen.Models) (int64, error) {
	var err error
...
// OneByID retrieves a row from 'models' as a Models.
//
// Generated from index 'primary'.
// nolint[goconst]
func (s *ModelsStore) OneByID(id int) (*codegen.Models, error) {
	s.where = "A.ID = $1"
	return s.One(id)
}
...
func (s *ModelsStore) ToJSON(t *sdb.JsonBuffer, data *Models) {
	prepend := "{"
	if s.colSet == nil || s.colSet.Bit(codegen.Models_ID) == 1 {
